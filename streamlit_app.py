...
if not os.path.exists(ZIP_PATH):
    ...
    if not os.path.exists(EXTRACT_DIR):
        with zipfile.ZipFile(ZIP_PATH, 'r') as zip_ref:
            ...
            st.write("Contents of the extracted subdirectory:")
            st.write(os.listdir(os.path.join(EXTRACT_DIR, 'Malaria Forecasting')))
    ...
    try:
        ...
        st.write("Available files in the extracted directory:")
    ...
    if st.button("Forecast Malaria Cases"):
        try:
            ...
            st.error(f"Invalid region or model type: {e}")
        except AttributeError as e:
            st.error(f"Model does not support the required method: {e}")
        except Exception as e:
